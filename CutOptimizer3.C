...
//TTree* myBsTree = (TTree*)MCPUFile->Get("BsTree");
 ...
 vector<TString> Cutvec; 
  ...
  // TFile* myCutFile = new TFile("CutOptFileMCBsNewTrig2.root");
   // TFile* myCutFile = new TFile("CutOptFiles/CutOptFileMCBdNewTrig.root"); //(old)
TFile *myCutFile = TFile::Open("CutOptFiles/CutOptFileBdMCFinal.root");
   TTree* mycutTree = (TTree*)myCutFile->Get("cutTree");		

    Int_t nentries = mycutTree->GetEntries("BpMass > 5.039 && BpMass < 5.52");
...
*/  
 ...
 TString Wtagcut = "&& BpMass > 5.039 && BpMass < 5.52 && PFMuon == 1 && ((BdFlavour == 1 && MuCharge == 1) || (BdFlavour == -1 && MuCharge == -1) )";

 TString Rtagcut = "&& BpMass > 5.039 && BpMass < 5.52 && PFMuon == 1 && ((BdFlavour == -1 && MuCharge == 1) || (BdFlavour == 1 && MuCharge == -1) )"; 	
   ...
   int counter =0; 	
 //CHECK THE LIMITS!
  for(double IP=0; IP<0.5; IP = IP+0.1){	// IP<0.1
     for(double dR=0; dR<1; dR = dR+0.1){     // dR >0.3 
	for(double pt=1.5; pt<4; pt = pt+0.1){  // Pt > 2.1
...
//  TFile *BpcutFile = TFile::Open("CutOptFileMCBpNewTrig.root"); 
...
cout << endl;
cout << "Bs MC sample " << endl;
cout << endl;

    TFile* BdFile = TFile::Open("CutOptFiles/CutOptFileBsMCFinal2.root");
TString BestCuts =" DeltaR>0.300000 && MuIP<0.300000 && MuPt>2.900000 ";
 
  //TFile* BdFile = new TFile("CutOptFileMCBsNewTrig2.root");
 //TFile *BdFile = TFile::Open("/tmp/terhi/MCBd2/CutOptFileMCBdNewTrig.root");
 TTree* BdTree = (TTree*)BdFile->Get("cutTree");

   TString WtagcutBd = BestCuts + "&& BpMass > 5.265 && BpMass < 5.475 && PFMuon==1 && ( ( BsFlavour == 1 && MuCharge == 1 ) || ( MuCharge == -1 && BsFlavour == -1  ) )"; 
   TString OptimizedCut = BestCuts + "&& BpMass > 5.265 && BpMass < 5.475 && PFMuon==1 && MuCharge!=0"; 
  ...
  cout << "total Bs " << TotalTagsBd << " " << "WtagsBs " << WtagsBd << endl;
  int entriesBd = BdTree->GetEntries("BpMass > 5.265 && BpMass < 5.475");
